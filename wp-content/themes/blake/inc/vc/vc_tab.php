...
$output .= '<div id="tab-'. (empty($tab_id) ? esc_attr(sanitize_title( $title )) : esc_attr($tab_id)) .'" class="'.esc_attr(trim($css_class)).'">';
$output .= ($content=='' || $content==' ') ? esc_html__("Empty tab. Edit page to add content here.", "blake") : "\n\t\t\t\t" . $content;
$output .= '</div> ';
