...
Class BlakeCustomPageManager{
	...
	function build_page(){
		...
		$this->templater=new BlakeTemplater();
		...
		echo wp_kses_post('<input type="hidden" value="'.esc_attr($this->taxonomy).'" id="taxnonomy_id" />
			  ...
			  <input type="hidden" value="'.esc_attr(wp_create_nonce(BLAKE_NONCE)).'" id="blake_nonce" />
		      </div>');
	}
	...
	function _print_items($category){
		...
		$ordered_posts=blake_get_ordered_post_list($posts, $category,$this->custom_page->post_type);
