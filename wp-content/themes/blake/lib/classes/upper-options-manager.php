...
class BlakeOptionsManager{
	...
	function print_heading($heading_text){
		...
		echo '<div id="blake-content-container"><form method="post" id="blake-options">';
		if ( function_exists('wp_nonce_field') ){
			wp_nonce_field('blake-theme-update-options','blake-theme-options');
		}
		...
		echo '</ul></div></div><div id="content"><div id="header"><h3 id="theme_name">'.esc_html($this->themename).' <span>v.'.esc_html($this->blake_version).'</span></h3><a class="online-doc" href="http://docs.upperinc.com/blake-wp/" target="_blank"><i class="fa fa-file-text-o"> </i>&nbsp; Online Documentation</a><a class="support" target="_blank" href="http://upperinc.com/support"><i class="fa fa-comments-o"></i> Help & Support Forum</a></div><input type="submit" value="Save Changes" class="save-button" /><div id="options_container">';
	}
	...
	function print_footer(){
		echo '</div></div><div id="blake-footer"><div id="follow-upper"> 
			 <p>Follow us</p><ul>
			 <li><a href="http://facebook.com/UpperInc" title="Upper on Facebook"><img src="'.esc_url($this->blake_images_url).'facebook.png" /></a></li>
			 <li><a href="http://twitter.com/UpperInc" title="Upper on Twitter"><img src="'.esc_html($this->blake_images_url).'twitter.png" /></a></li>
	...
	function close_option($value){
		if(isset($value['desc'])){
			echo '<a href="" class="help-button"><div class="help-dialog" title="'.esc_attr(esc_attr($value['name'])).'"><p>'.wp_kses_post($value['desc']).'</p></div></a>';
		}
	...
	function print_textarea($value){
		...
		$input_value = $this->get_field_value($value['id'], $std);
		if (isset($value['params']) && $value['params'] == 'blake_rich_editor'){
			?>
			<script src="<?php echo esc_url(get_template_directory_uri()); ?>/lib/classes/ace/min/ace.js" type="text/javascript" charset="utf-8"></script>
			<?php
...
jQuery("#'.esc_js($value['id']).'").html( jQuery("iframe#'.esc_js($value['id']).'_editor_ifr").contents().find("#tinymce.'.esc_js($value['id']).'_editor" ).html() ) });
	...
	function print_color($value){
		...
		$input_value = $this->get_field_value($value['id'], $std);
		echo '<span class="numbersign">&#35;</span><input class="option-input color" name="'.esc_attr($value['id']).'" id="'.esc_attr($value['id']).'" type="text" value="'.esc_attr($input_value).'" />';
		echo '<div class="color-preview" style="background-color:#'.esc_html($input_value).'"></div>';
		$this->close_option($value);
	}
	...
	function print_upload($value){
		...
		echo '<div id="'.esc_attr($value['id']).'_button" class="upload-button upload-logo" ><a class="blake-button alignright"><span class="upload-panel">Upload</span></a></div><br/>';
		$blake_admin_inline_script = (isset($blake_admin_inline_script)) ? $blake_admin_inline_script : "";
		$blake_admin_inline_script .= '
			jQuery(document).ready(function(){
				blakeOptions.loadUploader(jQuery("div#'.esc_js($value['id']).'_button"), "'.esc_js($this->blake_utils_url).'upload-handler.php", "'.esc_js($this->blake_uploads_url).'");
			});
		';
		wp_add_inline_script('blake-admin', $blake_admin_inline_script, 'after');
	...
	function print_stylebox($value, $type){
		...
		foreach ($value['options'] as $option) {
			...
			if($type=='pattern') {
				...
				if($option != "none")
					$style='background-image:url('.BLAKE_PATTERNS_URL.$option.');';
				else
			...
			echo '<li onclick="jQuery(this).parents(\'#tab_navigation-2-general\').find(\'#'."blake".'_style_color\').attr(\'value\',\''.esc_js($option).'\'); jQuery(this).parents(\'#tab_navigation-2-general\').find(\'.color-preview\').css(\'background-color\',\'#'.esc_js($option).'\');" style="'.esc_attr($style).'" class="'.esc_attr($class).'"><a class="style-box" title="'.esc_attr($option).'" href=""></a></li>';
		} 
		echo '</ul></div>';
		$this->close_option($value);
	}
	...
	function print_website_loaders($value){
		...
		foreach ($value['options'] as $option) {
			...
			if ($class == "load2" || $class == "load3" || $class == "load6"){
				echo '<div onclick="jQuery(this).closest(\'.loaders-styles-holder\').find(\'#'."blake".'_website_loader\').attr(\'value\',\''.esc_js($option['id']).'\'); jQuery(this).addClass(\'selected\').siblings().removeClass(\'selected\');" class="loader-item '.esc_attr($selected).'"><div class="loaders-style-box '.esc_attr($class).'"><div class="loader"></div></div></div>';
			} else {
				echo '<div onclick="jQuery(this).closest(\'.loaders-styles-holder\').find(\'#'."blake".'_website_loader\').attr(\'value\',\''.esc_js($option['id']).'\'); jQuery(this).addClass(\'selected\').siblings().removeClass(\'selected\');" class="loader-item '.esc_attr($selected).'"><div class="loaders-style-box loader-inner '.esc_attr($class).'">'.wp_kses_post($divs).'</div></div>';
			}
	...
	function print_custom($value){
		...
		foreach($value['fields'] as $field){
			...
			switch($field['type']){
				...
				case 'upload':
					...
					echo '<div id="'.esc_attr($field['id']).'_button" class="upload-button upload-logo" ><a class="blake-button alignright"><span>Upload</span></a></div><br/>';
					echo '<script type="text/javascript">jQuery(document).ready(function($){
								blakeOptions.loadUploader(jQuery("div#'.esc_js($field['id']).'_button"), "'.esc_js($this->blake_utils_url).'upload-handler.php", "'.esc_js($this->blake_uploads_url).'");
						});</script>';
					$preview=$field['id'];
					$is_textarea[]="false";
				break;
			...
			if ($field['id'] == "blake_google_fonts_name"){
				...
				if (count($fonts) > 1){
					...
					foreach($fonts as $f){
						if ($f != "")
							echo 'jQuery(\'#add_google_font_list\').append(\'<li><b>Font URL: </b><span class="blake_google_fonts_name_span">'.$f.'</span><br><div class="editButton hover"></div><div class="deleteButton hover"></div></li>\');';
					}
					echo '});
					</script>';	
				}
			...
			if ($field['id'] == "blake_sidebar_name_name"){
				...
				if (count($sidebars) > 0){
					...
					foreach($sidebars as $s){
						if ($s != ""){
							echo 'jQuery(\'#sidebar_name_list\').append(\'<li><b>Name: </b><span class="blake_sidebar_name_span">'.$s.'</span><br><div class="editButton hover"></div><div class="deleteButton hover"></div></li>\');';	
						}
					}
					echo '});
					</script>';	
		...
		echo '<a class="blake-button custom-option-button" id="'.esc_attr($value['id']).'_button"><span>'.wp_kses_post($value['button_text']).'</span></a>';
		
		//print the list that will contain the added items
		//if ($value['id'] != 'blake_reset_options_button')
		...
		echo '<script type="text/javascript">
			jQuery(document).ready(function($){
				blakeOptions.setCustomFieldsFunc("'.esc_js($value['id']).'", ["'.esc_js($idsString).'"], ["'.esc_js($namesString).'"], ['.esc_js($textareaString).'] , "'.esc_js($prv).'",  "");
				jQuery(\'#blake_export_options_button\').css({\'position\':\'relative\',\'float\':\'left\'}).attr(\'target\',\'_blank\').unbind().click(function(){
	...
	function print_goto($value){
		?>
		<div class="des_go_to button"><a target="_blank" href="<?php echo "themes.php?page=blake_style_options&dgtt=".esc_attr($value['name']); ?>"><?php echo esc_html($value['desc']); ?></a></div>
		<?php
	}
	...
	function print_mediauploader($value){
		$modal_update_href = esc_url( add_query_arg( array(
		    'page' => 'blake_gallery',
		    '_wpnonce' => wp_create_nonce('blake_gallery_options'),
